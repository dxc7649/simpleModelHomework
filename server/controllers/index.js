...
// function for adding new Dog's naem 
const setDogName = (req, res) => {
    if (!req.body.firstname || !req.body.lastname || !req.body.breed || !req.body.age) {
        return res.status(400).json({
            error: 'firstname, lastname, breed, and age are all required'
        });
    }
...
// Function to search for specific dog
const searchDog = (req, res) => {
    if (!req.body.name || !req.body.moreAge) {
        return res.status(400).json({
            error: 'Both name and Adding number need to be filled'
        });
    }

    return Dog.findDogByName(req.body.name, (err, doc) => {
        if (err) {
            return res.status(500).json({
                err
            }); // if error, return it
        ...
        if (!doc) {
            ...
            let addNumber = parseInt(req.body.moreAge);
            
            doc.age = doc.age + addNumber;
